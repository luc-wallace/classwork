...
def CreateNewField():
    ...
    RocksPlanted = 0
    while RocksPlanted < 5:
        RandRow = randint(0, FIELDLENGTH - 1)
        RandCol = randint(0, FIELDWIDTH - 1)
        if Field[RandRow][RandCol] == SOIL:
            Field[RandRow][RandCol] = ROCKS
            RocksPlanted += 1
...
def SimulateSpring(Field):
    for Row in range(FIELDLENGTH):
        for Column in range(FIELDWIDTH):
            if Field[Row][Column] == SEED and randint(1, 10) <= 4:
            ...
            for Column in range(FIELDWIDTH):
                if Field[Row][Column] == PLANT:
                    ...
                    if PlantCount % 2 == 0:
                        Field[Row][Column] = SOIL
                        PlantDeaths += 1
        print(f"There has been a severe drought: {PlantDeaths} plants have died")
        CountPlants(Field)
    return Field
